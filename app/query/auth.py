...
from app.db.models import Users, User_type
from app.schemas.auth import RegUser, UserInfo
from app.schemas.exception import UserFoundException, NotFoundException, ForbiddenException
...
async def check_role(nickname: str, session: AsyncSession):
    user_query = select(Users).where(Users.nickname == nickname)
    user: Users = await session.scalar(user_query)
    if not user:
        raise NotFoundException(error="Пользователь не найден")
    if user.user_type != User_type.Admin.name:
        raise ForbiddenException(error="Доступ закрыт")
